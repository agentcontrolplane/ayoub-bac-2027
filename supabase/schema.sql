...
create table if not exists public.profiles (id uuid primary key references auth.users(id) on delete cascade,email text unique not null,full_name text not null,role text not null check(role in('admin','student')),family_id text not null default 'ayoub-2027',created_at timestamptz default now());
create table if not exists public.weeks (id uuid primary key default gen_random_uuid(),family_id text not null default 'ayoub-2027',week_number int not null,title text not null,objective text,status text not null default 'planned' check(status in('planned','in_progress','validated','repeat_required')),admin_note text,student_note text,validated_at timestamptz,created_at timestamptz default now(),unique(family_id,week_number));
create table if not exists public.themes (id uuid primary key default gen_random_uuid(),family_id text not null default 'ayoub-2027',week_id uuid references public.weeks(id) on delete set null,subject text not null check(subject in('maths','physique','chimie','svt','informatique')),title text not null,description text,objective text,status text not null default 'assigned' check(status in('assigned','in_progress','done','validated','repeat_next_week')),priority text default 'normal',estimated_minutes int default 60,repeated_from uuid references public.themes(id),created_by uuid references public.profiles(id),created_at timestamptz default now());
...
drop policy if exists weeks_admin_all on public.weeks;create policy weeks_admin_all on public.weeks for all using(public.is_admin() and family_id=public.current_family_id()) with check(public.is_admin() and family_id=public.current_family_id());
drop policy if exists themes_read on public.themes;create policy themes_read on public.themes for select using(public.is_member() and family_id=public.current_family_id());
drop policy if exists themes_admin_all on public.themes;create policy themes_admin_all on public.themes for all using(public.is_admin() and family_id=public.current_family_id()) with check(public.is_admin() and family_id=public.current_family_id());
drop policy if exists tasks_read on public.tasks;create policy tasks_read on public.tasks for select using(public.is_member() and family_id=public.current_family_id());
drop policy if exists tasks_admin_all on public.tasks;create policy tasks_admin_all on public.tasks for all using(public.is_admin() and family_id=public.current_family_id()) with check(public.is_admin() and family_id=public.current_family_id());
drop policy if exists tasks_student_update on public.tasks;create policy tasks_student_update on public.tasks for update using(public.current_role()='student' and family_id=public.current_family_id()) with check(public.current_role()='student' and family_id=public.current_family_id() and status in('assigned','done_by_ayoub'));
drop policy if exists resources_read on public.resources;create policy resources_read on public.resources for select using(public.is_member() and family_id=public.current_family_id());
...
drop policy if exists ayoub_files_read on storage.objects;create policy ayoub_files_read on storage.objects for select using(bucket_id='ayoub-files' and public.is_member());
drop policy if exists ayoub_files_admin_insert on storage.objects;create policy ayoub_files_admin_insert on storage.objects for insert with check(bucket_id='ayoub-files' and public.is_admin());
drop policy if exists ayoub_files_admin_update on storage.objects;create policy ayoub_files_admin_update on storage.objects for update using(bucket_id='ayoub-files' and public.is_admin()) with check(bucket_id='ayoub-files' and public.is_admin());
drop policy if exists ayoub_files_admin_delete on storage.objects;create policy ayoub_files_admin_delete on storage.objects for delete using(bucket_id='ayoub-files' and public.is_admin());

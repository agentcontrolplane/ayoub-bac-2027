  ...
  return (
    <div className="app">
      <aside className="sidebar">
        <div className="brand"><span>⚽</span><div><b>Mission Bac 2027</b><small>{isAdmin ? 'Accès Oussama/Asma' : 'Accès Ayoub'}</small></div></div>
...
function Planning({data, profile, isAdmin, isStudent, setMessage, reload}){
  ...
  async function markDone(task){ const comment = prompt('Commentaire pour Oussama/Asma : qu’as-tu fait ? Où as-tu bloqué ?') || ''; const {error}=await supabase.from('tasks').update({status:'done_by_ayoub',student_comment:comment}).eq('id',task.id); if(error)setMessage(error.message); else reload(); }
